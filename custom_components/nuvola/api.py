...
class NuvolaAPI:
    ...
    async def login(self):
        ...
        async with s.get(
            ...
            final_url = str(r.url)
            _LOGGER.warning(
                "[0.8.3 DIAG] Nuvola OIDC Keycloak login page: HTTP %s final_url=%s type=%s title=%s diagnostics=%s",
                r.status,
            ...
            _LOGGER.warning(
                "[0.8.3 DIAG] Nuvola OIDC Keycloak form: action=%s input_names=%s username_field=%s password_field_present=%s parser_form=%s candidates=%d page_has_session_code=%s",
            ...
            _LOGGER.warning(
                "[0.8.3 DIAG] Nuvola OIDC Keycloak action query keys: %s full_session_action=%s",
                action_query_keys,
                has_full_action,
            )
        ...
        async with s.post(
            ...
            final_url = str(r.url)
            _LOGGER.warning(
                "[0.8.3 DIAG] Nuvola OIDC credential submit: HTTP %s final_url=%s type=%s title=%s cookies=%s",
                r.status,
                self._safe_url(final_url),
                r.headers.get("Content-Type"),
        ...
        async with s.get(
            ...
            final_url = str(r.url)
            _LOGGER.warning(
                "[0.8.3 DIAG] Nuvola OIDC /area-tutore: HTTP %s final_url=%s type=%s title=%s cookies=%s",
                r.status,
                self._safe_url(final_url),
                r.headers.get("Content-Type"),
        ...
        async with s.get(
            ...
            final_url = str(r.url)
            _LOGGER.warning(
                "[0.8.3 DIAG] Nuvola OIDC login-from-web: HTTP %s final_url=%s type=%s title=%s",
                r.status,
                self._safe_url(final_url),
                r.headers.get("Content-Type"),
                self._safe_title(body),
    ...
    async def _json(self, path, params=None):
        ...
        for attempt in range(2):
            async with s.get(
                ...
                body = await self._text(r, 12000)
                _LOGGER.warning(
                    "[0.8.3 DIAG] API request: path=%s attempt=%d HTTP=%s content_type=%s body_preview=%r",
                ...
                if r.status == 401:
                    ...
                    _LOGGER.error(
                        "[0.8.3 DIAG] Nuvola API failed after token handling: path=%s HTTP=%s body=%r",
                ...
                try:
                    ...
                    _LOGGER.warning("[0.8.3 DIAG] API JSON parsed: path=%s shape=%s", path, shape)
                    return data
                except json.JSONDecodeError as err:
                    raise NuvolaAuthError(
    ...
    async def bulletin_documents(self, student_id):
        ...
        data = await self._json(
            f"/api-studente/v1/alunno/{student_id}/documenti",
    ...
    async def students(self):
        ...
        _LOGGER.warning(
            "[0.8.3 DIAG] students: extracted %d records from response",
            len(values),
        )
        return values
    ...
    async def periods(self, student_id):
        ...
        _LOGGER.warning(
            "[0.8.3 DIAG] periods: extracted %d records",
            len(values),
        )
        return values
    ...
    async def fetch_all(self):
        _LOGGER.warning("[0.8.3 DIAG] fetch_all: START")
        students = await self.students()
        _LOGGER.warning(
            "[0.8.3 DIAG] fetch_all: students result count=%d first_keys=%s",
        ...
        if not students:
            _LOGGER.error("[0.8.3 DIAG] fetch_all: ZERO STUDENTS -> all sensors will remain 0")
            return result

        sid = students[0].get("id") or students[0].get("id_alunno")
        if sid is None:
            _LOGGER.error("[0.8.3 DIAG] fetch_all: student found but no id/id_alunno field -> all child API calls skipped")
            return result
        ...
        try:
            ...
            _LOGGER.warning("[0.8.3 DIAG] periods unavailable: %s", err)
        ...
        if result["periods"]:
            ...
            if pid is not None:
                try:
                    ...
                    _LOGGER.warning("[0.8.3 DIAG] grades unavailable: %s", err)

        try:
            result["absences"] = await self.absences(sid)
        except Exception as err:
            _LOGGER.warning("[0.8.3 DIAG] absences unavailable: %s", err)

        try:
            result["notes"] = await self.notes(sid)
        except Exception as err:
            _LOGGER.warning("[0.8.3 DIAG] notes unavailable: %s", err)

        try:
            result["homework"] = await self.homework(sid)
        except Exception as err:
            _LOGGER.warning("[0.8.3 DIAG] homework unavailable: %s", err)

        try:
            result["bulletin_documents"] = await self.bulletin_documents(sid)
        except Exception as err:
            _LOGGER.warning("[0.8.3 DIAG] bulletin documents unavailable: %s", err)
            result["bulletin_documents"] = []

        _LOGGER.warning(
            "[0.8.3 DIAG] fetch_all: END students=%d periods=%d grades_type=%s absences_type=%s notes_type=%s homework=%d bulletin=%d",
